...
const AddTrackForm = ({ handleOpen, setIsOpen }: FormUpdateBookDataProps) => {
    // Images
    ...
    const formSchema = z.object({
        track_name: z.string().min(2, {
            message: "Nama jalur must be at least 2 characters.",
        }),
        image: z
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        try {
            ...
            const { data: imageData, error: imageError } = await supabase.storage.from("trackimages").upload(`image-${values.track_name}-${uniqueID}`, imageFile, {
                cacheControl: "3600",
                upsert: false,
            })
            if (imageError) {
            ...
            // insert data
            const { error: supabaseError } = await supabase.from("track_data").insert({
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem className="w-full">
                                <FormLabel className="text-neutral-500">Nama Jalur</FormLabel>
                                <FormControl>
                                    <Input  {...field} />
                                </FormControl>
                                <FormMessage />
